package abdul.restApi.spring.webflux.integration.controller.admin;
...
public class UserControllerV1Test {
    ...
    void saveUserTestController() {
        ...
        webTestClient.post().uri("/api/v1/users/").header("Authorization", "Bearer " + jwtToken).contentType(MediaType.APPLICATION_JSON).bodyValue(json).exchange().expectStatus().isCreated();
    ...
    void updateUserTestController() {
        ...
        webTestClient.put().uri("/api/v1/users/").header("Authorization", "Bearer " + jwtToken).contentType(MediaType.APPLICATION_JSON).bodyValue(json).exchange().expectStatus().isOk();
    }
    ...
    void deleteUserTestController() {
        webTestClient.delete().uri("/api/v1/users/2").header("Authorization", "Bearer " + jwtToken).exchange().expectStatus().isOk();
    }
}
