...
public class EventControllerV1Test {
    ...
    void updateEventTestController() {
        ...
        webTestClient.put().uri("/api/v1/events/").header("Authorization", "Bearer " + jwtToken).bodyValue(eventDto).exchange().expectStatus().isForbidden();
    }
    ...
    void getAllEventsTestController() {
        webTestClient.get().uri("/api/v1/events/").header("Authorization", "Bearer " + jwtToken).exchange().expectStatus().isForbidden();
    }
    ...
    void deleteUserTestController() {
        webTestClient.delete().uri("/api/v1/events/2").header("Authorization", "Bearer " + jwtToken).exchange().expectStatus().isForbidden();
    }
}
